...
def classifyPerson():
    label_list = ['not at all', 'in small dose', 'in large dose']
    games = float(raw_input('percentage of time playing video game?'))
    fmiles = float(raw_input('frequent flyer miles earned per year'))
    ice_cream = float(raw_input('liters of ice cream consumed per week?'))
    X,Y = file2matrix('datingTestSet2.txt')
    norm_X, ranges, min_vals = auto_norm(X)
    x = np.array([fmiles,games,ice_cream])
    x = (x-min_vals)/ranges
    guess = classify0(x, norm_X, Y, 3)
    print 'you will prabably like this person : ', label_list[guess - 1]



def img2vector(filename):
    ret_vec = np.xeros((1, 1024))
    fr = open(filename)
    for i in range(32):
        line = fr.readline()
        for j in range(32):
...
def hwClassTest():
    hw_labels = []
    train file_list = listdir('trainingDigits')
    n = len(train)